      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="max-w-3xl mx-auto text-center mb-16">
          <div className="inline-flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-bold tracking-widest text-[var(--primary-blue)] uppercase">Product Categories</span>
            <span className="w-6 h-px bg-[var(--primary-blue)]"></span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
          ...
          <div className="lg:col-span-7 bg-[var(--pearl-white)] border border-[var(--visible-border)] p-6 sm:p-8 md:p-10 flex flex-col justify-between shadow-lg relative text-left">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {/* Visual Image Banner */}
                <div className="h-[200px] w-full overflow-hidden border border-[var(--visible-border)] relative">
                  <img
                  ...
                  />
                  <div className="absolute top-4 left-4 bg-[var(--primary-blue)] text-white px-3 py-1 text-[10px] font-bold tracking-widest uppercase">
                ...
                {/* Info Header */}
                <div>
                  <h3 className="text-xl sm:text-2xl font-extrabold text-[var(--primary-blue)]">{activeCategory.name}</h3>
                  <p className="text-[var(--muted-text)] font-light text-xs sm:text-sm leading-relaxed mt-2">
                    {activeCategory.description}
                  </p>
                </div>

                {/* Compliance & Operations Info Specs */}
                <div className="bg-[var(--pearl-white)] p-5 border-l-4 border-[var(--primary-blue)] grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <span className="text-[9px] uppercase font-bold text-[var(--muted-text)] tracking-wider">Estimated Listing Time</span>
                    <p className="text-sm font-bold text-[var(--primary-blue)] mt-0.5">{activeInsight.listingTime}</p>
                  </div>
                  <div>
                    <span className="text-[9px] uppercase font-bold text-[var(--muted-text)] tracking-wider">Priority Regulatory Agency</span>
                    <p className="text-sm font-bold text-[var(--primary-blue)] mt-0.5 break-words">{activeInsight.priorityAgency}</p>
                  </div>
                  <div>
                    <span className="text-[9px] uppercase font-bold text-[var(--muted-text)] tracking-wider">Key Growth Metric</span>
                    <p className="text-sm font-bold text-[var(--dark-navy)] mt-0.5">{activeInsight.keySuccessMetric}</p>
                  </div>
                </div>

                {/* Category success factors list */}
                <div className="space-y-2">
                  <h5 className="text-[10px] uppercase font-bold text-[var(--primary-blue)] tracking-widest">In-Market Representation Scope:</h5>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-[var(--dark-navy)] font-light">
            ...
            <div className="mt-8 pt-6 border-t border-[var(--visible-border)] flex flex-col sm:flex-row items-center justify-between gap-4">
              <span className="text-xs text-[var(--muted-text)] font-light">Need a direct shelf space assessment for this category?</span>
              <button
                onClick={() => onBookClick(`Hello, I would like to inquire about your specialized support for the: "${activeCategory.name}" product category.`)}
                className="w-full sm:w-auto bg-[var(--primary-blue)] hover:bg-[var(--primary-blue-hover)] text-white text-xs font-bold uppercase tracking-wider py-3.5 px-6 transition-colors shadow-sm"
              >
                Start Your Growth
              </button>
            </div>
